...
class SecuritySystem:
    def __init__(self, config: Dict[str, Any]):
        ...
        # Sync stealth mode to actuators
        self.actuators.stealth_mode = self.stealth_mode
...
def api_stealth():
    ...
    # Sync stealth mode to actuators
    system.actuators.stealth_mode = system.stealth_mode
    
    # Update LED immediately based on current mode
    if system.stealth_mode:
        # Force LED off in stealth mode
        system.led_state = LEDState.OFF
        system.actuators.led_off()
    else:
        # Restore LED state based on mode
        if system.mode == SystemMode.ARMED:
            system.led_state = LEDState.SOLID
        elif system.mode == SystemMode.DISARMED:
            system.led_state = LEDState.SLOW_BLINK
        elif system.mode == SystemMode.ALARM:
            system.led_state = LEDState.FAST_BLINK
...
def api_test():
    ...
    try:
        # Enable manual override for manual controls (bypasses stealth mode)
        system.actuators.manual_override = True
        
        if act == "servo":
            if val == "lock": 
                system.actuators.lock_box()
            else: 
                system.actuators.unlock_box()
        elif act == "buzzer":
            ...
            success, message = system._take_photo("Manual Test")
            if not success:
                return jsonify({"success": False, "error": message, "message": "Camera unavailable or disconnected"})
        
        # Disable manual override after test
        system.actuators.manual_override = False
        
        return jsonify({"success": True})
    except Exception as e:
        logging.error(f"Test actuator failed: {e}")
        system.actuators.manual_override = False  # Ensure it's reset on error
        return jsonify({"success": False, "error": str(e), "message": f"Failed to activate {act}"})
...
def api_get_settings():
    ...
    try:
        return jsonify({
            "success": True,
            "settings": {
                "logic": system.config.get("logic", {}),
                "camera": system.config.get("camera", {}),
                "servo_angle": system.config.get("servo_angle", {})
            }
        })
    except Exception as e:
...
def api_save_settings():
    ...
    try:
        ...
        if "logic" in data:
            system.config["logic"].update(data["logic"])
        if "camera" in data:
            system.config["camera"].update(data["camera"])
        if "servo_angle" in data:
            system.config["servo_angle"].update(data["servo_angle"])
        ...
        with open(full_path, "w") as f:
            json.dump(system.config, f, indent=2)
        
        logging.info("Settings saved successfully")
        system.log_event("SETTINGS", "Configuration updated")
