...
class Actuators:
    def __init__(self, led_bcm: int, buzzer_bcm: int, servo_bcm: int):
        ...
        # Stealth mode control - will be set by SecuritySystem
        self.stealth_mode = False
        self.manual_override = False  # Allow manual testing
    ...
    def buzz_once(self, duration=0.05):
        ...
        # Check stealth mode - don't buzz unless manual override
        if self.stealth_mode and not self.manual_override:
            logging.info("Actuators: Buzzer muted (stealth mode)")
            return
    ...
    def alarm_active(self, state: bool):
        ...
        # Check stealth mode - don't activate alarm unless manual override
        if state and self.stealth_mode and not self.manual_override:
            logging.info("Actuators: Alarm muted (stealth mode)")
            return
    ...
    def led_on(self):
        # Check stealth mode - don't turn on LED unless manual override
        if self.stealth_mode and not self.manual_override:
            logging.debug("Actuators: LED muted (stealth mode)")
            return
        self.led.on()
